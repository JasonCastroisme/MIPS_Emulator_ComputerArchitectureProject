...
void CPU::decode() {
  ...
  opcode = (instr >> 26) & 0x3f;//Shifting opcode and masking it by 0x3f to isolate opcode
  rs = (instr >> 21) & 0x1f;//Shifting rs register and masking it by 0x1f to isolate rs register
  rt = (instr >> 16) & 0x1f;//Shifting rt register and masking it by 0x1f to isolate rt register
  rd = (instr >> 11) & 0x1f;//Shifting rd register and masking it by 0x1f to isolate rd register
  shamt = (instr >> 6) & 0x1f; //Shifting shamt register and masking it by 0x1f to isolate shamt register
  funct = (instr & 0x3f); //Shifting funct register and masking it by 0x3f to isolate funct register
  uimm = (instr & 0xffff);//unsigned number masking for positive numbers only, reference by green sheet as ZeroExtImm
  simm = ((signed((instr & 0xffff)<<16)) >> 16) ;// Signed immediates, shifting left 16 to pull the signed bit copies over after shifting it right 16; for SignExtImm
  addr = (instr & 0x03ffffff);//bits 0-26 masked to grab address
